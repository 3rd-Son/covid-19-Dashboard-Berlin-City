...
def run():
    ...
    link = (
        "[©Developed by Vi3sax](https://www.linkedin.com/in/victory-nnaji-8186231b7/)"
    )
...
def get_data():
    ...
    historic_district_cases = pd.read_csv(
        historic_district_cases_url, sep=";", encoding="unicode_escape"
    )
...
historic_district_cases_df["Datum"] = pd.to_datetime(
    historic_district_cases_df["Datum"]
)
...
selected_districts = st.sidebar.multiselect(
    ...
    default=[
        "Lichtenberg"
    ],  # setting Lichtenberg as the default district as it's the one I'm most interested in seeing
)

if selected_districts == []:
    selected_districts = [
        "All Berlin"
    ]  # if all selected districts are x'd out, display data for 'All Berlin'
...
nocyber = st.sidebar.checkbox("LIGHT THEME")
...
for i in selected_districts:
    new_reported_cases = pd.concat(
        [new_reported_cases, historic_district_cases_df[i]], axis=1
    )
...
for i in selected_districts:
    new_reported_cases["Seven Day Sum for %s" % i] = (
        new_reported_cases[i].rolling(7).sum()
    )
...
for i in selected_districts:
    ...
    new_reported_cases["Seven Day Incidence for %s" % i] = (
        new_reported_cases["Seven Day Sum for %s" % i] / popn
    )
...
for i in selected_districts:
    incidence = pd.concat(
        [incidence, new_reported_cases["Seven Day Incidence for %s" % i]], axis=1
    )
...
st.write(
    "This chart shows the 7 day incidence (# of cases per 100,000 inhabitants) for the selected district(s)"
)
...
st.write(
    "This chart shows a [rolling 7-day-average](https://en.wikipedia.org/wiki/Moving_average) of newly reported cases for the selected district(s)."
)
st.write(
    "This smoothes out the spikes somewhat and makes it easier to identify the real trend in cases."
)
...
st.write(
    "This chart shows the raw number of new reported cases in the selected district(s)."
)
st.write(
    "This will show larger variance and generally be 'noisier' than the 7-day-average chart."
)
st.write(
    "Notice that the numbers tend to dip to near zero on weekends and spike on Mondays. This is an artifact of the data collection process and not a real trend - new cases are generally not recorded / reported over weekends."
)
...
# Displaying the plot and the last 3 days' values
st.pyplot(fig)
number_to_limit_table = (
    len(selected_districts) + 1
)  # This is just a hack to display the figures I want
...
st.write(
    ...
    See the code on [GitHub](https://github.com/Vic3sax/covid-19-Dashboard-Berlin-City).
